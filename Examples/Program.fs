...
let main argv =
    ...
    *)
        (*
        let outputResult = wrapper.InvokeFunction "main" inputDataSmall :?> single array
        printfn "%A" outputResult
        let outputResult = wrapper.InvokeFunction "main" inputDataMedium :?> single array
        printfn "%A" outputResult
        *)
        
        (*
        let outputResult = wrapper.InvokeFunction "main" locVolLarge :?> single array
        printfn "%A" outputResult
        0
        *)
    ...
    (*
    let compiledNbodyTest =
        let clock = new Stopwatch()
        clock.Start()
        let nbody = new NBodyExample([||])
        clock.Stop()
        printfn "Opening class from assembly took %i ms" (FShark.Library.Utils.TicksToMicroseconds clock.ElapsedTicks)
        
        let foo = fun i ->
            clock.Restart()
            ignore <| nbody.main(1000, 50.0f, 0.1f, tupleFromArray xpsIn,tupleFromArray ypsIn, tupleFromArray zpsIn, tupleFromArray msIn, tupleFromArray xvsIn, tupleFromArray yvsIn, tupleFromArray zvsIn, tupleFromArray xasIn, tupleFromArray yasIn, tupleFromArray zasIn)
            clock.Stop()
        ...
        printfn "Running nbody from assembly took %i ms" average
        0
