...
module FSharkCompiler = 
    ...
    and GetFSharkCall (f : FSharpMemberOrFunctionOrValue) tps args : FSharkCode =
        ...
        let fname' = match GetContainingModule f.ApparentEnclosingEntity with
                   | None -> fname
                   | Some value -> value ^ "." ^ fname
        let args' = List.map GetFSharkCode args
        in FSharkCode.Call (fname', args')
        
    and GetContainingModule (e : FSharpEntity) : string option =
        if e.AccessPath = "global" then None
        else 
            let fullName = e.FullName
            let modules = String.concat "." (Array.tail <| fullName.Split [|'.'|])
            in Some modules
